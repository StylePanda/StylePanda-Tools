...
(async function () {
  ...
  // Shared result layout: bounding-box regression across every required width.
  for (const width of [320, 375, 390, 430, 768, 1024, 1280, 1440, 1920]) {
    await command('Emulation.setDeviceMetricsOverride', { width, height: 1100, deviceScaleFactor: 1, mobile: width < 768 }); await navigate('komprimieren', 'compress'); await evaluate(`${makeFiles}(1)`, true); await waitFor("document.querySelector('[data-workspace]').hidden === false"); await evaluate("document.querySelector('[data-process]').click(); true"); await waitFor("Number(document.querySelector('[data-image-tool]').dataset.resultSize) > 0");
    const layout = await evaluate(`(() => { const panel = document.querySelector('[data-result-box]'); const preview = panel.querySelector('[data-result-preview]').closest('.image-preview'); const metrics = panel.querySelector('.stats-grid'); const cards = Array.from(metrics.querySelectorAll('.stat-card')); const button = panel.querySelector('[data-download]'); const rect = element => { const value = element.getBoundingClientRect(); return { left: value.left, top: value.top, right: value.right, bottom: value.bottom, width: value.width, height: value.height }; }; const overlap = (a, b) => a.left < b.right && a.right > b.left && a.top < b.bottom && a.bottom > b.top; const cardRects = cards.map(rect); return { panel: rect(panel), preview: rect(preview), metrics: rect(metrics), button: rect(button), cards: cards.map((card, index) => { const value = card.querySelector('strong'); const valueRect = rect(value); return { text: value.textContent, width: cardRects[index].width, valueWidth: valueRect.width, valueHeight: valueRect.height, lineHeight: parseFloat(getComputedStyle(value).lineHeight), contentOverflow: card.scrollWidth > card.clientWidth }; }), overlap: cardRects.some((card, index) => cardRects.slice(index + 1).some(other => overlap(card, other))), bodyOverflow: document.documentElement.scrollWidth > document.documentElement.clientWidth }; })()`);
    assert.ok(layout.panel.width > 0 && layout.preview.width > 0 && layout.metrics.width > 0, `${width}px: Ergebnis, Vorschau und Metriken sichtbar`); assert.strictEqual(layout.cards.length, 3, `${width}px: drei Metrikkarten`); layout.cards.forEach(card => { assert.ok(card.width >= 110, `${width}px: Metrikkarte nicht kollabiert (${JSON.stringify(card)})`); assert.ok(card.valueWidth >= 70, `${width}px: Messwert besitzt lesbare Breite (${JSON.stringify(card)})`); assert.ok(card.valueHeight <= card.lineHeight * 1.6, `${width}px: Messwert bleibt einzeilig (${JSON.stringify(card)})`); assert.strictEqual(card.contentOverflow, false, `${width}px: Messwert läuft nicht aus der Karte (${JSON.stringify(card)})`); }); assert.strictEqual(layout.overlap, false, `${width}px: Metrikkarten überlappen nicht`); assert.ok(layout.button.left >= layout.panel.left && layout.button.right <= layout.panel.right && layout.button.top >= layout.panel.top && layout.button.bottom <= layout.panel.bottom, `${width}px: Download-Schaltfläche innerhalb des Ergebnisbereichs`); assert.ok(layout.preview.left >= layout.panel.left && layout.preview.right <= layout.panel.right, `${width}px: Vorschau innerhalb des Ergebnisbereichs`); assert.strictEqual(layout.bodyOverflow, false, `${width}px: kein horizontaler Dokument-Overflow`);
  }
  ...
  console.log(`IMAGE-PAGES-BROWSER ERFOLGREICH: 10 Seiten, PNG/JPEG/WebP-Produktionsdecoder, 21 UI-Formatläufe, Offline-Kernfunktionen, Ergebnis-Bounding-Boxes, Stapel/ZIP, Fehlerfall und 9 Viewports`); socket.close();
